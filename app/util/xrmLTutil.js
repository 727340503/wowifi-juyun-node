...
exports.updUserInfo = function(data,cb){
	...
	if(data.new_recordtype != null && data.new_recordtype != undefined){
	...
	if(data.new_type != undefined && data.new_type != null){
		filter += ',{"Key":"new_type","Value":"'+data.new_type+'"}';
	}	
	var args = {};
	args.entityname = 'new_jifei_customer';
	args.data = '['+filter+']';
...
exports.createLTOrder = function(data,cb){
	...
	//用户的订单失效时间计算完毕
	var requestJson = JSON.stringify(data.reqData);
	...
	args.data = '['+
	            ...
	            '{"Key":"new_jifei_requestdetail","Value":"'+requestJson+'"},'+
	            ']';
	console.log(args);
	client.Create(args,function(err,result){
		if(err){
...
exports.createInvalidOrder = function(data,cb){
	var client = data.client;
	var LTPackageId = settings.LTPACKAGEID;
	var num = data.num;
	...
	args.data = '[{"Key":"new_jifei_customerid","Value":"'+userId+'"},'+
						...
						'{"Key":"new_jifei_packageid","Value":"'+LTPackageId+'"},'+
