	        ...
	        client.Query(args,function(err,result){
	        	...
	        	if(off){
	        		...
	        		if(resultData.new_jifei_customerid && resultData.new_jifei_customerid != ''){
		        		...
		        		console.log("增加前时长:"+allTime);
		        		// allTime += 8*60*60*1000;
		        		console.log("增加后时长:"+allTime);
		        		if(allTime < 0){
		        			allTime = 0;
		        		}
		        		if(resultData.issuccess != undefined || resultData.issuccess == false){
							...
							async.auto({
								qryPageOrder:function(callback){
									var qryHistoyArgs = {};
				        			...
				        			client.Query(qryHistoyArgs,function(err,result){
				        				...
				        				if(off){
				        					...
				        					if(historyData.issuccess != undefined || historyData.issuccess == false){
				        						return res.json({resultCode:'0',time:Math.floor(allTime/1000)+'',order_length:'0',data:"",allpage:'0'});
				        					}
				        					callback(null,historyData);
				        				}else{
				        					res.json({resultCode:'0',time:+Math.floor(allTime/1000)+'',order_length:'0',data:"",allpage:'0'});
				        				}
				        			});
								},
								qryAllPage:['qryPageOrder',function(callback,result){
									//查询订单总页数
		        					var qryHistoyAllPage = {};
				        			qryHistoyAllPage.entityname = 'new_jifei_alipay';
							        qryHistoyAllPage.filter = '[{"Logical":"and","Conditions":[{"Key":"new_jifei_customerid","Operator":"=","Value":"'+userId+'"}'+filter+']}]';
							        qryHistoyAllPage.fields = 'createdon';
						          	qryHistoyAllPage.orderby = 'createdon desc';
							        qryHistoyAllPage.pagesize = 100000000;
							        qryHistoyAllPage.pageindex = 1;
							        console.log(result);
							        console.log('**********************开始查询总页数**********************');
		        					client.Query(qryHistoyAllPage,function(err,result){
	        							if(err){
				        					console.log(err);
				        					console.log('***************查询用户充值历史总页数出错****************');
				        					return res.json({resultCode:'1',info:'查询用户充值历史总页数异常,请稍候再试'});
				        				}
				        				var off = null; 
				        				for(var a in result.QueryResult){
				        					if(a){
				        				...
				        				if(off){
				        					var resultData = {};
				        					var allPageData = JSON.parse(result.QueryResult);
				        					resultData.off= off;
				        					resultData.data = allPageData;
								...
								var historyData = results.qryPageOrder;
								var allPageData = results.qryAllPage;
								//如果总页数查询没有结果
								if(allPageData.off){
									if(allPageData.issuccess != undefined || allPageData.issuccess == false){
										return res.json({resultCode:'0',time:Math.floor(allTime/1000)+'',order_length:historyData.length+'',data:historyData,allpage:'0'});
									}
									console.log(allPageData.data);
									var allOrder = allPageData.data.length;
									var allPage = Math.ceil(allOrder/pagesize);
									res.json({resultCode:'0',time:Math.floor(allTime/1000)+'',order_length:historyData.length+'',data:historyData,allpage:allPage+''});
								}else{
									res.json({resultCode:'0',time:Math.floor(allTime/1000)+'',order_length:historyData.length+'',data:historyData,allpage:'0'});
								}
							});
		        		}
	        		}else{
