...
exports.sendMsg = function(req,res){
	...
	try{
		...
		if(methodCode == 1){
			...
			smsTemplateCode = settings.SMS_REGISTID;
			methodName = "注册帐号";
		}else if(methodCode == 2){
			smsFreeSignName = "变更验证";
			smsTemplateCode = settings.SMS_UPDATEPWD;
			methodName = "修改密码";
		}else if(methodCode == 3){
			smsFreeSignName = "身份验证";
			smsTemplateCode = settings.SMS_AUTHENTICATION;
			methodName = "身份验证";
		}
		var smsmsg_url = settings.SMSMESSAGE_URL;
		var smsParam = '{"code":"'+code+'","product":"wowifi"}';
		var url =smsmsg_url+"?smsFreeSignName="+smsFreeSignName+"&smsParam="+smsParam+"&recNum="+phone+"&smsTemplateCode="+smsTemplateCode;
		url = encodeURI(url);
