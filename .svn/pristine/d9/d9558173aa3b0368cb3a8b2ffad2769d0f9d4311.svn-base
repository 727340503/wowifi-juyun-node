            ...
            if(off == null){
                ...
                if(data.OPTYPE==1){
                    ...
                    if(updData.statecode == 1){//判断企业状态
                        return res.send({'issuccess':true,'data':'企业已经处于暂停状态'});
                    }
                    ...
                    LTUtil.factoryUsers(updData,function(num){
                        if(num != 0){
                            ...
                            LTUtil.editFactoryState(updData,function(flag){
                                if(flag){
                                    var _err = {'issuccess':true,'data':'企业暂停成功'};
                                    res.send(_err);
                                }else{
                                    res.send({'issuccess':false,'data':'企业暂停失败'});                            
                                }
                            }); 
                        }
                    });
                    ...
                    var updData = JSON.parse(result.QueryResult)[0];
                    if(updData.statecode == 0){//判断企业状态
                        return res.send({'issuccess':true,'data':'企业已经恢复'});
                    }
                    updData.state = 0;
                    updData.client = client;
                    LTUtil.editFactoryState(updData,function(flag){
                        if(flag){
                            var _err = {'issuccess':true,'data':'企业恢复成功'};
                            res.send(_err);
                        }else{
                            res.send({'issuccess':false,'data':'企业恢复失败'});                            
                        }
                    });
                }else if(data.OPTYPE==4){
                    res.send({'issuccess':false,'data':'企业更改信息失败'});
            ...
            if(off){
                ...
                async.auto({
                    qryUserInfo : function(callback){//查询用户个人信息
                        ...
                        LTUtil.getUserInfo(qryData,function(userInfo){//查询用户信息 
                            ...
                            if(userInfo.resultCode == 0){//用户存在，则继续下一步
                                ...
                                //创建用户
                                if(params.OPTYPE != 1 || params.OPTYPE == 2){//判断用户是订购还是退订,如果是退订,可以返回信息给请求方
                    ...
                    qryUserOrder: ['qryUserInfo',function(callback,result){
                        ...
                        LTUtil.qryOrder(qryData,function(orderInfo){
                            if(orderInfo.resultCode == 0){//如果用户订单存在，则继续下一步
                                ...
                                //创建用户创建失效订单
                                // if(params.OPTYPE != 1 || params.OPTYPE == 2){//判断用户是订购还是退订,如果是退订,可以返回信息给请求方
                                //     return res.send({'issuccess':true,'data':'对不起,该用户的订单不存在'});
                                // }
                                var createData = {};
                                createData.client = client;
                                var num = 1;
                    ...
                    soap.createClient(url2,function(err,client2){
                        ...
                        }else if(params.OPTYPE == 2){//用户暂停
                            if(userInfoObj.statecode == 1){//判断用户状态是否已经是暂停
                                res.send({'issuccess':true,'data':'用户已经暂停'});
                            }else{
                                //判断用户状态是否属于禁用,如果否更改状态为禁用
                                var updUserData = {};
                                updUserData.state = 1;
                                updUserData.userInfoObj = userInfoObj;
                                LTUtil.editUserState(updUserData,function(flag){
                                    if(flag){
                                        res.send({'issuccess':false,'data':'用户暂停成功'});
                                    }else{
                                        res.send({'issuccess':false,'data':'用户暂停失败'});
                                    }
                                });
                            }
                        }else if(params.OPTYPE == 3){//用户恢复
                            if(userInfoObj.statecode == 1){//判断用户状态是否是退订
                                //判断用户状态是否属于禁用,如果是更改状态为可用
                                var updUserData = {};
                                updUserData.state = 0;
                                updUserData.userInfoObj = userInfoObj;
                                LTUtil.editUserState(updUserData,function(flag){
                                    if(flag){
                                        res.send({'issuccess':true,'data':'用户恢复成功'});
                                    }else{
                                        res.send({'issuccess':false,'data':'用户恢复失败'});
                                    }
                                });
                            }else{
                                res.send({'issuccess':true,'data':'用户已经恢复'});
                            }
                        }else if(params.OPTYPE == 4){//用户信息变更
                            res.send({'issuccess':false,'data':'用户信息更新失败'});
