    ...
    soap.createClient(url,function(err,client){
    	...
    	async.auto({
			...
			updAlipayArgs.data =  '[{"Key":"new_jifei_alipayid","Value":"'+order_no+'"},'+
								  ...
								  '{"Key":"new_jifei_validtime","Value":"'+Time.format(new Date(orderSumBeginTime).toLocaleString(),'yyyy-MM-dd HH:mm:ss')+'"},'+//订单开始生效的时间
			...
			client.Update(updAlipayArgs,function(err,result){
				...
				if(off){
					...
					if(updalipayData.issuccess){
						...
						createOrderData.new_jifei_begintime = Time.format(new Date(orderSumBeginTime).toLocaleString(),'yyyy-MM-dd HH:mm:ss');
						...
						createOrderData.new_jifei_packageid = aipayData.new_jifei_packagetempid;
						console.log(createOrderData);
						Utils.createOrderSum(createOrderData,function(flag){
							if(flag){//总表添加成功,更新订单管理表
								var updArgs = {};
								updArgs.client = client;
								updArgs.orderId = orderData.new_jifei_orderid;
